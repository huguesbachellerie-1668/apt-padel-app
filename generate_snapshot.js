...
async function main() {
  ...
  let md = `# Classement Final - Saison 2025-2026\n\n`;
  md += `| Rang | Joueur | Moyenne pts | Sessions | Matchs | Points Totaux | Tops | Flops |\n`;
  md += `|------|--------|-------------|----------|--------|---------------|------|-------|\n`;
  
  ranking.forEach((r, i) => {
    const displayName = r.nickname ? `${r.nickname} (${r.name})` : r.name;
    const avg = r.averagePoints.toFixed(2);
    md += `| ${i + 1} | ${displayName} | **${avg}** | ${r.sessionsCount} | ${r.totalMatches} | ${Math.floor(r.points)} | ${r.tops} | ${r.flops} |\n`;
  });
